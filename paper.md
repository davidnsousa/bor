...
``bor`` is an R [@R] package developed to transform raw focal sampling data of social interaction events into asymmetric data matrices (one for each type of social interaction recorded). For example, the number of times (events) a chimpanzee observes other group members. Focal sampling is a widely used sampling method in observational studies [@altmann1974observational; @lener1998handbook; @martin1993measuring].  When referring to social interactions, focal samples detail with whom the focal subject interacted with, the type of social interactions that took place, and who was responsible for initiating these interactions.
